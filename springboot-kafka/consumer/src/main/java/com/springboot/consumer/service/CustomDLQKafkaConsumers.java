...
@Service
@Component
public class CustomDLQKafkaConsumers {

    private static final Logger logger = LoggerFactory.getLogger(CustomDLQKafkaConsumers.class);
        ...
        try {
            ...
            if (originalTopicHeader != null) {
                ...
                if (retryCount < 5) {
                    ...
                    logger.error("[Paytm] | [MQ] | [CustomDLQKafkaConsumers] | [dlq-topic-consumer] Retry limit exceeded for message {}", json);
                }
            } else {
    ...
    @KafkaListener(topics = "dlq-topic", groupId = "dlq-topic-group", containerFactory = "kafkaListenerContainerFactory")
